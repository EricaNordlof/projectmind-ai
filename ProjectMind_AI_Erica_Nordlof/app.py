...
PERSISTENT_DISK = os.getenv("PERSISTENT_DISK", "false").lower() == "true"
...
USING_POSTGRES = DATABASE_URL.startswith(("postgresql://", "postgresql+psycopg://"))


USING_LOCAL_PERSISTENT_DISK = (
    STORAGE_BACKEND == "local"
    and PERSISTENT_DISK
    and str(DATA_DIR).startswith("/app/storage")
)

PERSISTENCE_OK = (
    (USING_POSTGRES and STORAGE_BACKEND == "s3")
    or USING_LOCAL_PERSISTENT_DISK
)

if USING_POSTGRES and STORAGE_BACKEND == "s3":
    PERSISTENCE_MODE = "production"
    PERSISTENCE_TITLE = "Produktionslagring aktiv"
    PERSISTENCE_MESSAGE = (
        "Projektdata sparas i PostgreSQL och uppladdade filer i privat "
        "S3-kompatibel objektlagring."
    )
elif USING_LOCAL_PERSISTENT_DISK:
    PERSISTENCE_MODE = "persistent_disk"
    PERSISTENCE_TITLE = "Permanent lagring aktiv"
    PERSISTENCE_MESSAGE = (
        "Projekt, chattar och uppladdade filer sparas på den anslutna "
        "Render-disken under /app/storage och finns kvar efter deployer "
        "och omstarter."
    )
else:
    PERSISTENCE_MODE = "development"
    PERSISTENCE_TITLE = "Lagringen är inte permanent"
    PERSISTENCE_MESSAGE = (
        "Appen använder lokal lagring utan bekräftad persistent disk. "
        "Data kan försvinna vid omstart eller deploy."
    )
...
def _sql(sql: str) -> str:
    """Translate the app's SQLite-style ? placeholders for PostgreSQL."""
...
def _s3_client():
    ...
    if not S3_BUCKET or not AWS_ACCESS_KEY_ID or not AWS_SECRET_ACCESS_KEY:
        raise RuntimeError(
            "S3-lagring är vald men S3_BUCKET/AWS_ACCESS_KEY_ID/"
            "AWS_SECRET_ACCESS_KEY saknas."
        )
    config = None
...
def local_storage_path(kind: str, owner_id: str, stored_name: str) -> Path:
    base = {
        "uploads": UPLOAD_DIR,
        "versions": VERSION_DIR,
        "chat_uploads": CHAT_UPLOAD_DIR,
    }[kind]
    return base / owner_id / stored_name
...
async def save_upload(
    ...
    try:
        with temp:
            while True:
                ...
                if total > limit:
                    raise HTTPException(
                        status_code=413,
                        detail=f"Filen är större än {limit // (1024 * 1024)} MB.",
                    )
                temp.write(chunk)
        await upload.close()
        ...
        if STORAGE_BACKEND == "s3":
            ...
            return total, temp_path

        destination = local_storage_path(kind, owner_id, stored_name)
...
def storage_exists(kind: str, owner_id: str, stored_name: str) -> bool:
    if STORAGE_BACKEND == "s3":
        try:
            ...
            return False
    return local_storage_path(kind, owner_id, stored_name).exists()


def storage_bytes(kind: str, owner_id: str, stored_name: str) -> bytes:
    if STORAGE_BACKEND == "s3":
    ...
    path = local_storage_path(kind, owner_id, stored_name)
    if not path.exists():
...
def storage_download_response(
    ...
    if STORAGE_BACKEND == "s3":
        ...
        headers = {
            "Content-Disposition": f'attachment; filename="{safe_filename(original_name)}"',
        }
...
def storage_delete(kind: str, owner_id: str, stored_name: str) -> None:
    if STORAGE_BACKEND == "s3":
        ...
        return
    local_storage_path(kind, owner_id, stored_name).unlink(missing_ok=True)


def storage_delete_prefix(kind: str, owner_id: str) -> None:
    if STORAGE_BACKEND == "s3":
        client = _s3_client()
        ...
        return
    base = {"uploads": UPLOAD_DIR, "versions": VERSION_DIR, "chat_uploads": CHAT_UPLOAD_DIR}[kind]
    shutil.rmtree(base / owner_id, ignore_errors=True)
...
def image_to_data_url(record: dict[str, Any]) -> str:
    try:
        raw = storage_bytes("chat_uploads", record["chat_id"], record["stored_name"])
    except FileNotFoundError as exc:
        raise RuntimeError(
            f"Den bifogade bilden {record['original_name']} saknas på lagringen."
        ) from exc
...
def health():
    ...
    try:
        ...
        error = f"{error}; storage: {exc}".strip("; ")
    ...
    return render(
        ...
        storage_backend=(
            ...
            if STORAGE_BACKEND == "s3"
                ...
                if USING_LOCAL_PERSISTENT_DISK
                else "lokal tillfällig disk"
            )
        ),
    )
...
def delete_project(request: Request, project_id: str, csrf: Annotated[str, Form(...)]):
    ...
    for item in chat_ids:
        storage_delete_prefix("chat_uploads", item["id"])
...
async def upload_project_file(
    ...
    size, _ = await save_upload(upload, "uploads", project_id, stored)
...
async def upload_version(
    ...
    stored = f"{uid()}_{original}"
    size, temporary_path = await save_upload(
        upload,
        "versions",
        project_id,
        stored,
    ...
    try:
        zip_source = temporary_path
        if STORAGE_BACKEND == "s3":
            zip_source = Path(tempfile.mkstemp(suffix=".zip")[1])
            zip_source.write_bytes(storage_bytes("versions", project_id, stored))
        ...
        with zipfile.ZipFile(zip_source) as zf:
            for info in zf.infolist():
                ...
                if candidate.is_absolute() or ".." in candidate.parts:
                    ...
                    raise HTTPException(
                        status_code=400,
                        detail="ZIP-filen innehåller en osäker sökväg.",
                    )
    except zipfile.BadZipFile as exc:
        storage_delete("versions", project_id, stored)
        raise HTTPException(status_code=400, detail="Ogiltig ZIP-fil.") from exc
    finally:
        if STORAGE_BACKEND == "s3" and zip_source:
            zip_source.unlink(missing_ok=True)
        if temporary_path and temporary_path.exists() and STORAGE_BACKEND == "s3":
...
async def send_message(
    ...
    try:
        for upload in real_images:
            ...
            stored = f"{uid()}_{original}"
            total, temporary_path = await save_upload(
                upload,
                "chat_uploads",
                chat_id,
                stored,
            ...
            if temporary_path and STORAGE_BACKEND == "s3":
                temporary_path.unlink(missing_ok=True)

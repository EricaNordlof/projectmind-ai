...
async def ask_ai(history: list[dict[str, str]], context: str) -> str:
...
def chat_page(request: Request, chat_id: str):
    ...
    project = fetchone("SELECT * FROM projects WHERE id=?", (chat["project_id"],)) if chat["project_id"] else None
    messages = fetchall("SELECT * FROM messages WHERE chat_id=? ORDER BY created_at ASC", (chat_id,))
    projects = fetchall("SELECT id,name FROM projects ORDER BY updated_at DESC")
    return render(request, "chat.html", chat=chat, project=project, messages=messages, projects=projects)
...
async def send_message(
    ...
    execute(
        ...
        (uid(), chat_id, "user", user_text, now()),
    )
    history = fetchall("SELECT role,content FROM messages WHERE chat_id=? ORDER BY created_at ASC", (chat_id,))
    try:
        answer = await ask_ai(history, project_context(chat["project_id"]))
